...
class ROSTopicRouter {
  ...
  connect_bidirectional() {
    ...
    this.pub_topic.subscribe(function(message) {
      ...
      // document.getElementById('topic_status_' + num).style.color = "green";
      var msg_str = JSON.stringify(message);
      if (this.recently_sent_from_sub.indexOf(msg_str) < 0) {
        this.sub_topic.publish(message);
        this.recently_sent_from_pub.push(msg_str);
        if (this.recently_sent_from_pub.length > 10) {
          this.recently_sent_from_pub.shift();
        }
      }
    });
  }
